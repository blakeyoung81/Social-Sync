...
def cut_silence_auto_editor(input_path: Path, output_path: Path, threshold_str: str, margin: float = 0.2) -> bool:
    ...
    # We will use a fixed, reliable value for now. The value from the UI is for ffmpeg.
    auto_editor_threshold = 0.04 
    logger.warning(f"  [Silence Cut] Using default auto-editor threshold ({auto_editor_threshold}). FFmpeg threshold '{threshold_str}' from UI is ignored.")
...
def find_key_moments_with_gpt(transcript_text: str, video_topic: str = "general", openai_api_key: str = None, custom_prompt: str = None) -> dict:
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are an educational content expert who identifies the most important words and phrases to highlight for maximum learning engagement and retention."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.2  # Lower temperature for more consistent educational focus
        )
        ...
        # Combine keywords and emphasis words for comprehensive highlighting
        all_keywords = keywords + emphasis_words
        
        if isinstance(highlights, list) and isinstance(all_keywords, list):
            logger.info(f"  [AI Highlights] Found {len(highlights)} key learning sentences and {len(all_keywords)} educational terms to highlight.")
            logger.info(f"  [AI Highlights] Educational terms: {', '.join(all_keywords[:5])}...")
...
def add_outro_ffmpeg(video_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        # Scale outro to match main video size and concatenate
        concat_cmd = [
            "ffmpeg", "-y",
            "-i", str(video_path),
            "-i", str(outro_path),
...
def add_topic_card(video_path: Path, output_path: Path, topic: str, card_duration: float = 3.0, style: str = 'medical', position: str = 'top') -> bool:
    ...
    try:
        ...
        logger.error(f"  [Topic Card] Failed to create topic card: {e}", exc_info=True)
        return False
...
def add_colorful_frame(video_path: Path, output_path: Path, frame_style: str = 'rainbow') -> bool:
    ...
    try:
        ...
        logger.error(f"  [Colorful Frame] Error adding frame: {e}")
        # Copy the original video file to the output path to maintain workflow
        try:
            shutil.copy(str(video_path), str(output_path))
...
def get_broll_keywords_with_gpt(transcript_text: str, openai_api_key: str = None) -> list[str]:
    ...
    prompt = f"""
    Analyze the following transcript from a medical education video.
    Identify 3-5 distinct, visually representable concepts or nouns that would be good for B-roll footage.
    Focus on concrete things, not abstract ideas. For example, "stethoscope," "pills," "doctor writing," "heart animation," "lab test."
...
def insert_broll_clips(
    ...
    try:
        ...
        logger.error(f"  [B-Roll] Error during B-roll insertion: {e}", exc_info=True)
        # Cleanup
...
def apply_dynamic_zoom(video_path: Path, output_path: Path, intensity: str = 'subtle', frequency: str = 'medium') -> bool:
    ...
    try:
        ...
        logger.error(f"  [Zoom] Error applying dynamic zoom: {e}", exc_info=True)
        return False
    ...
    try:
        ...
        for attempt in range(retries):
            try:
                ...
                logger.error(f"  [Music] Error during attempt {attempt + 1}: {e}")
        ...
        if music_clip.duration < video_duration:
            ...
            if video_duration <= music_clip.duration * 2:
                # If we need less than 2 loops, just extend and cut
        ...
        logger.error(f"  [Music] Error during background music addition: {e}")
        try:
            shutil.copy(str(video_path), str(output_path))
...
def process_video(
    ...
    music_background_volume: float = 0.15,
    ...
    **kwargs
    ...
    try:
        ...
        if not skip_silence:
            ...
            if cut_silence_auto_editor(current_video_path, silence_cut_path, threshold_str=silence_threshold, margin=silence_duration):
        ...
        if not skip_broll and transcript_path.exists():
        ...
        if (not skip_ai_highlights or use_ai_word_highlighting) and transcript_path.exists():
            ...
            if key_moments and use_ai_word_highlighting:
                ass_path = TEMP_PROCESSING_DIR / f"highlighted_{current_video_path.stem}.ass"
                if convert_srt_to_ass_with_highlights(transcript_path, ass_path, key_moments, highlight_style):
                    # The highlights are burned along with subtitles later
                    logger.info(f"✅ AI word highlighting generated and saved to '{ass_path.name}'.")
                else:
                ...
                logger.info("No key moments found for highlighting.")
        ...
        if (not skip_broll or not skip_image_generation) and transcript_path.exists():
            ...
            try:
                ...
                pexels_api_key=pexels_api_key,
                    ...
                    broll_clip_count=broll_clip_count,
                ...
                print(f"🎬 ERROR: Multimedia processing failed: {e}")
                logger.error(f"Multimedia processing failed: {e}")
        else:
        ...
        if not skip_sound_effects:
            ...
            try:
                ...
                logger.info("✅ Sound effects added successfully.")
                    ...
                    logger.warning("Sound effects not added or failed, continuing with original video.")

            except Exception as e:
                logger.error(f"Sound effects processing failed: {e}", exc_info=True)
                logger.warning("Sound effects not added, continuing with original video.")
            finally:
        ...
        logger.error(f"FATAL: An unexpected error in process_video for {input_file_path.name}: {e}", exc_info=True)
        return None
    finally:
...
def insert_smart_broll_clips(
    ...
    try:
        ...
        logger.error(f"  [AI B-Roll] Error during intelligent B-roll insertion: {e}")
        # Cleanup
        if main_clip: main_clip.close()
        for clip in opened_broll_clips: clip.close()
        return video_path
...
def analyze_transcript_for_broll_suggestions(transcript_text: str, video_duration: float, detected_topic: str, openai_api_key: str = None, custom_prompt: str = None) -> dict:
    ...
    if custom_prompt:
        ...
        default_prompt = DEFAULT_SETTINGS["gptPrompts"]["multimediaAnalysis"]
        # Extract just the B-roll portion or create a focused B-roll prompt
        prompt = f"""
